...
public class searchServer {
	...
	public searchServer(){
     ...
     public searchServer(String server, String q, String auth, String attrib){
    	 this.urlServer=server;
    	 this.q=q;
    	 this.authlist=auth;
    	 this.attrib=attrib;
     }
	public void returnInformation() throws Exception {

		searchServer http = new searchServer();
		//String urlServer="http://betamasaheft.aai.uni-hamburg.de/";
		//String url = urlServer+"api/works/list";
		String url=urlServer+"api/search/"+attrib+"?collection="+authlist+"&q="+q;
	   ...
	   //do 
		//for(int j=start;j<stop;j++){
			...
			for (int i=0;i<doc.getRootElement().getChildren().size()-1;i++){
			    ...
			    String attrRole=doc.getRootElement().getChildren().get(i).getAttributeValue("role");
